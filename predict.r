rm(list=ls());
source("functions.r");
args = commandArgs(trailingOnly = TRUE);train = args[1];task = args[2];pred_risk = args[3];pred_days = args[4];
seed=1; # Seed is set for replication of results. Set this to NA if want more randomness.
